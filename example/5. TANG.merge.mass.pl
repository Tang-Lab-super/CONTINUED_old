...
#Congratulations on reaching the integration and annotation stage!
#Before integrating, make sure you have the input files for each sample. Note that three files should
#be placed in each sample folder: '_signal.lock_mass.txt', '_signal.unlock_mass.txt' and '_signal.selected.coor.txt'.

#Run in linux as the ./TANG.merge.mass.pl command. You can get a 'colon_cancer_desi.clustered_mass.table.with.anno.txt' finally.
#A list of 'plot.kde.index_{index}.txt' will also be generated.

my $output_prefix = 'colon_cancer_desi_';

#Create a new file named 'sample.list.selected.txt' that each line is the name of each sample, such as 'ST87_20210331'.
#Fill in the name of the sample you want to integrate.
my $input_sample_list = '/data/bingling/rPackageTutorial/Continued/example/Dataset/sample.list.selected.txt';
my $work_dir = '/data/tang/desi/input/combined';  #Home directory of all sample input files.

#LC-MS datasets include lipid and small molecules.
my $input_lipid = '/data/bingling/rPackageTutorial/Continued/example/Dataset/LC-MS/20210930.Lipid.8_samples.uniq.txt';
my $input_small_mol = '/data/bingling/rPackageTutorial/Continued/example/Dataset/LC-MS/20220107.combined.small_molecule.neg.uniq.txt';
...
#	while( my $e = shift @{ $pd } ){
#	    if( $pd->[0] ){
#		if( $e->[1] > $pd_max->[1] && ! $pd_min_right ){
#		    $pd_max = $e;
#		}
#		elsif ( $e->[1] < $pd_max->[1] && ! $pd_min_right ){
#		    $pd_min_right = $e;
#		}
#		elsif ( $e->[1] == $pd_max->[1] ){
#		    $pd_min_right =$e;
#		}
#		elsif ( $e->[1] < $pd_max->[1] &&  $e->[1] > $pd->[0]->[1] && $pd_min_right ){
#		    $pd_min_right =$e;
#		}
#		elsif ( $e->[1] < $pd->[0]->[1] && $pd_min_right ){
#		    push @regions, [$pd_min_left, $pd_min_right, $pd_max];
#		    undef $pd_min_right;
#		    $pd_min_left = $e;
#		    $pd_max = $pd_min_left;
...
# https://metacpan.org/pod/Statistics::KernelEstimation
